...
```{r metaregression data setup for size categorisation at study level - data setup}
...
dat.size.metareg <- left_join(dat5.metareg, dat10.metareg) %>%
  mutate(prop_size = total_size5 / total_size10)
...
size.metareg = metaprop(rupt10, total_size10,
                      data=dat.size.metareg,
                      studlab=paste(auth_year),
                      method="GLMM",
                      sm="PLOGIT",
                      method.tau = "ML", 
...
r3 <- metareg(size.metareg, ~prop_size)
r3
```
...
```{r metaregression - option 2 - bubble plot - size}

bubble(r3, studlab=dat.size.metareg$auth_year, xlim=c(0, 1), main="Meta-regression plot for Aneurysm size")
...
```{r metaregression for for study end year - execution}
...
r4 <- metareg(year.metareg, ~end)
r4
```
 
```{r metaregression - option 2 - bubble plot - year}

bubble(r4, studlab=dat.year.metareg$auth_year, xlim=c(1960, 2020), main="Meta-regression plot for Study End Year")
