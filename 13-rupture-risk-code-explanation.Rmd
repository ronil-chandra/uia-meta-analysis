---
title: "Rupture Risk Data Analysis Rationale and Explanation"
...
* The distribution for the outcome of rupture is binomial. The GLMM accounts for the binomial structure of the data and thus avoids the generic inverse variance method.
...
### Display all included information for all aneurysms 10 mm and less - option 1. 
...
tab1 <- sizedata %>%
  as_hux() %>%
  theme_article() %>%
  set_caption("Study Characteristics") %>%
  select(auth, pub, start, end, country, type, num_anr, psah, fu, rupt, rupt_psah, nos_total) 
...
sizedata_ht <- as_hux(sizedata)

width(sizedata_ht) <- 0.4

sizedata_ht <- sizedata_ht %>%
  select(auth, pub, country, type, num_anr, rupt, rupt_psah, fu, nos_total) %>%
  arrange(auth) %>%
  rename("Author" = auth, "Publication Year" = pub, "Country" = country, "Study Type" = type, "Number of Aneurysms" = num_anr, "Number of Ruptured Aneurysms" = rupt, "Number of Ruptured Aneursysms with Prior SAH" = rupt_psah, "Follow up" = fu, "Newcastle-Ottowa Score" = nos_total) %>%
  huxtable::add_colnames() %>%
  set_number_format(0) %>%
  set_align(1:4, 2, 'right') %>%
  set_caption("Characteristics of included studies") %>%
  set_caption_pos("topleft") %>%
  theme_article() %>%
  add_footnote("*All aneurysms measuring 10 mm and less")
...
**Does the pooled proportion use the Wilson method?**
...
5. Post hoc metagression to consider whether duration of follow up analysed as a continuous variable was an important source of heterogeneity. 

To reduce the risk of false positives, and to ensure that there are adequate studies to consider pos hoc meta-regression, only one posthoc potential effect modifier of duration of follow up is chosen, based on the clinical hypotheses that longer follow up durations may be required for rupture events to occur. 
...
Overall, there are still high levels of heterogeneity in the results, and relatively high degrees of imprecision in the pooled risk of rupture. 
...
Exploratory meta-regression to explore length of follow up on rupture outcomes


```{r metaregression data setup for length of follow up}
...
note that in option 1, I cannot produce a bubble plot, an alternate way to examine this is by doing a meta-analyis of proportions using metaprop, and then using metareg command on the output.
...
```{r metaregression exploration - option 2}

r1 <- metareg(fu.metareg, ~fu)
r1
```

```{r metaregression - option 2 - bubble plot}
...
```{r metaregression data setup for prior SAH at study level - data setup}
...
```{r metaregression data setup for prior SAH at study level - execution}
...
r2 <- metareg(psah.metareg, ~prop)
r2
```


```{r metaregression - option 2 - bubble plot - psah}

bubble(r2, studlab=dat.psah.metareg$auth_year, xlim=c(0, 1), main="Meta-regression plot for prior SAH")
...
#### Rerun forest plots excluding Juvela for a visual display of the sensitivity analysis for total cohort
...
#### Sensitivity analysis - exploratory - leave each study out 

note that traditional leave-1-out testing jn metafor does not apply to generalized linear mixed models, and thus individual studies are left out, and the summary pooled proportion re-calculated. 

An example below is performed for study 1, this will be repeated for all 30 studies to assess the impact on the pooled effect size. For the interest of space for initial draft code review, this is not included below. 

```{r leave 1 out sensitivity analysis - study 1}

dat.1 <- slice(dat, -1)
pes.summary.glmm.1 = metaprop(rupt, total_size,
                            data=dat.1,
                            studlab=paste(auth_year),
                            sm="PLOGIT",
                            method.tau = "ML", 
