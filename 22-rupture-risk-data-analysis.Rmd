...
The random effects metaregression included participants from 22 studies with a mean study level proportion of 5mm aneurysms of 76.9% (range 24.8 - 100%). There were a total of 144 ruptures and 8 914 aneurysms in studies included in this metaregression analysis. The metaregression result was not significant (p=0.83), suggesting that the proportion of patients with 5mm and less aneurysms included in the study did not modify the risk of rupture at the study level. However, there is moderate unexplained residual heterogeneity (I2 = 43%), which similar to all included studies, indicating that this meta-regression analysis was not informative in identifying sources of heterogeneity in the meta-analysis result. Regardless, this random effects metaregression analysis is susceptible to confounding bias from other study level characteristics, and is not generalisable to an individual patient, since there is aggregation bias due to study-level data being used for analysis.


#### Metaregression to explore size categorised at 3mm

```{r data setup for size categorisation at 3 mm at study level}

dat3.metareg <- dat3 %>%
  rename(total_size3 = total_size) %>%
  rename(rupt3 = rupt) %>%
  select(auth_year, rupt3, total_size3)

dat.size3.metareg <- left_join(dat3.metareg, dat10.metareg) %>%
  mutate(prop_size3 = total_size3 / total_size10) %>%
  select(auth_year, rupt10, total_size10, prop_size3)
...
```{r metaregression for 3mm category at study level, all studies - charachteristics}

range(dat.size3.metareg$prop_size3) * 100

mean(dat.size3.metareg$prop_size3) * 100

sum(dat.size3.metareg$rupt10)

sum(dat.size3.metareg$total_size10)
...
The random effects metaregression included participants from 15 studies with a mean study level proportion of 3mm aneurysms of 44.4% (range 8.2 - 100%). There were a total of 28 ruptures and 2 569 aneurysms in studies included in this metaregression analysis. The metaregression result was not significant (p=0.55), suggesting that the proportion of patients with 3mm and less aneurysms included in the study did not modify the risk of rupture at the study level. Although there is no unexplained residual heterogeneity (I2 = 0%), there are far less studies, ruptures and numbers of aneurysms included in this analysis, and thus it seems unlikely that this meta-regression analysis can be relied upon to produce valid results. 
...
4. Post hoc sensitivity analysis to consider the decision to exclude outlier studies. 


In the main analysis, we have included participants from studies where the proportion of patients with exposure to prior subarachnoid haemorrhage ranged from 0-50%. We can perform a sensitivity analysis to include only studies where the proportion of patients with no exposure to prior SAH is 95% or more. This is important from a clinical perspective given that majority of aneurysms are discovered incidentally without exposure to prior SAH.
...
#### Sensitivity analysis - consider handling of baseline imbalance in exposure to prior SAH

In the main analysis, we have included participants from studies where the proportion of patients with exposure to prior subarachnoid haemorrhage ranged from 0-50%. We can perform a sensitivity analysis to include only studies where the proportion of patients with no exposure to prior SAH is 95% or more. This is important from a clinical perspective given that majority of aneurysms are discovered incidentally without exposure to prior SAH. 

```{r sensitivity analysis - reduce exposure to prior SAH at study level to less than 5%}

dat.psah.sens <- dat10 %>%
  filter(prop_psah < 0.05)

pes.summary.psah.sens = metaprop(rupt, total_size,
                            data=dat.psah.sens,
                            studlab=paste(auth_year),
                            method="GLMM",
                            sm="PLOGIT",
...
pes.summary.psah.sens

```


```{r calculate mean FU of included studies of prior SAH exposure reduced to less than 5%}
...
Sensitivity analysis performed with aneurysms included in studies where over 95% of study participants did not have exposure to prior SAH has a pooled rupture risk estimate of 0.8459 [0.4149; 1.7171] over 41 months, with I^2 = 75.1%. 

Since the pooled rupture risk remains within 1-2%, this sensitivity analysis does not alter the overall impact from a clinical perspective. However, the inconsistency in the pooled result has increased, which reduces its validity. 
...
#### Sensitivity analysis - random vs fixed 
