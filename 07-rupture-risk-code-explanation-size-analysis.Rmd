...
pdf(file='forestplot_ruptureproportion7.pdf', width=10, height=8) 
...
pdf(file='forestplot_ruptureproportion5.pdf', width=10, height=8) 
...
Note that the Fisher scoring algorith fails to converge to calculate the tau2 using the maximum-likelihood estimator after the default 100 iterations; this also fails after attempting 1000 iterations. This can be corrected by adjusting reducing the step length, and the model converges. 
...
pdf(file='forestplot_ruptureproportion3.pdf', width=10, height=8) 
...
A subgroup analysis like we have done above using size categories can be conidered a meta-regression with categorical predictors. However, meta-regression allows us to utilise both categorical and continuous data as potential predictors, and check whether they modify outcome. 

In our case, we can now explore the influence of study follow up duration on the rupture proportion. 
...
```{r forest for psahpos using GLMM, fig.height= 10, fig.width = 10}

pdf(file='forestplot_ruptureproportionpsahpos.pdf', width=10, height=8) 
...
pdf(file='forestplot_ruptureproportionpsahposexcJuvela.pdf', width=10, height=8) 
...
```{r simple forest using glmm model for summary proportion without history of PSAH  }

pdf(file='forestplot_ruptureproportionpsahneg.pdf', width=10, height=8) 
forest(pes.summary.glmm.psahneg,
       layout = "meta",
       comb.fixed = FALSE,
       comb.random = TRUE,
       leftlabs = c("Study", "Ruptures", "Total"),
...
Now that the column with length of follow up is created, we can perform a subgroup analyis of studies with intermediate and long term follow up. 
...
pdf(file='forestplot_ruptureproportionint.pdf', width=10, height=8) 
...
pdf(file='forestplot_ruptureproportionlong.pdf', width=10, height=8) 
...
ci = patients without prior history of SAH who were included in studies with greater than 5 years of follow up who susbsequently ruptured. See that only 2 patients without history of prior SAH ruptured on long term follow up. Most of the results on long term follow up are patients with prior SAH who ruptured. 

This is the gap in the literature. 

There is minimal data to guide clinicians on the risk of rupture associated with conservative management of an incidental aneurysm beyond 5 years of follow up. 
