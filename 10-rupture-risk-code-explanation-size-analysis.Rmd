...
#### Rerun forest plots excluing Juvela which is the study that adds the most to heterogeniety for a sensitivity analysis for total cohort
...
```{r restructure to 2 x 2 table for PSAH 2}
...
```{r new tibbles for psah postive and psah negative 2 }
...
```{r glmm for summary proportion with history of PSAH 2}
...
```{r glmm for summary proportion without history of prior SAH 2}
...
```{r glmm for summary proportion with history of PSAH without Juvela 2}
...
```{r forest for psahpos using GLMM with Juvela, fig.height= 10, fig.width = 10 2}
...
```{r simple forest using glmm model for summary proportion with history of PSAH without Juvela, fig.height= 10, fig.width = 10 2}
...
```{r simple forest using glmm model for summary proportion without history of PSAH, fig.height= 10, fig.width = 10 2}
...
```{r create new column with length of follow up 2}
...
```{r subgroup analysis with intermediate of follow up 2}
...
```{r forest for publication using GLMM less than 5 years , fig.height= 10, fig.width = 10 2}
...
```{r meta-analysis of proportions using glmm long 2}
...
```{r forest for publication using GLMM long, fig.height= 10, fig.width = 10 2}
...
```{r restructure to 2 x 2 table for PSAH and long term follow up 2}
