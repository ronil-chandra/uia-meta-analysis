...
The purpose of this project is to improve the precision, or understand the imprecision, of the risk estimate for rupture of a brain aneurysm managed with active surveillance. This will help clinicians better convey the rupture risks to patients, and help on the journey toward personalised and patient centred decision making. 

My goal is to learn how to conduct a quantitative synthesis to better understand the current literature on the rupture of an intracranial aneurysm with conservative managemen
...
There are various strategies to explore and adddress heterogeneity. Remember that clinical and methodological heterogeneity always occur in a meta-analysis, and thus statistical heterogeneity is inevitable (Higgins et al 2003).

If there is considerable statistical heterogeniety, there are 2 options. 
...
#### Find outliers and influential studies
...
These plots proposed by Viechtbauer & Cheung (2010) demonstrate that the Juvela study is not only a statistical outlier, but also an influential study. This is important because this further corroborates that the Juvela study is responsible for the between group heterogenieity, reducing the robustnesss of the pooled result. 
...
In these 2 plots, we can see the impact of the leave 1 out analysis on the pooled effect size. The first plot is ordered by effect size, and the second by I2. 

We can see that the precision of the pooled effect size estimate improved with exlcusion of Juvela et al, and that the I2 reduces the most with exclusion of Juvela et al. 

These findings corroborate the findings of the outlier analysis, the Baujat plots, and influence analysis  proposed by Viechtbauer and Cheung. 

Since the outlier and influence analysis are concordant that Juvela is the main source of heterogeniety, is an outlier which influences the pooled effect estimate and reduces the precision of the pooled effect estimate, the analysis should exclude this study based on grounds of clinical and statistical heterogeneity.   

### Addressing heterogeniety 

There are several strategies to deal with heterogeniety
...
2. If numbe of patients with prior SAH is known, then this is multiplied by the multiplicity proportion to calculate number of aneurysms with exposure to prior SAH in the specific size criteria. 
...
psah.metareg.juvela = metaprop(rupt, total_size,
...
psah.metareg.result.juvela <- metareg(psah.metareg.juvela, ~prop_psah)
