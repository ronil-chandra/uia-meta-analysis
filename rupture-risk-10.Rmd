...
# Using the metabin function from meta package to asess the OR for prior SAH

Structure the data in this format

Author	This signifies the column for the study label (i.e., the first author)

Ee	Number of events in the exposure/treatment arm = rupture in PSAH cohort 
Ne	Number of participants in exposure/treatment arm arm = number of aneurysms in PSAH cohort
Ec	Number of events in the non-exposure/control arm = ruptures in non-PSAH cohort
Nc	Number of participants in the non-exposure/control arm = number of aneurysms in non-PSAH cohort. 
...
psah.or <- sizedata %>%
  ...
  rename(Ne = num_anr_psah) %>%
  mutate(Nc = total_anr - Ne) %>%
  select(auth, Ee, Ne, Ec, Nc) %>%
  drop_na(Ee, Ne) %>%
...
psah.or <- sizedata %>%
  ...
  mutate(prop_psah = psah_tot / num_tot) %>%
  mutate(num_anr_psah = prop_psah * total_anr) %>%
  mutate(bi = num_anr_psah - ai) %>%
  mutate(di = total_anr - ci) %>%
  select(auth, ai, bi, ci, di) %>%
  drop_na(ai, ci) %>%
  mutate_if(is.numeric, round, 0)
view(psah.or)
