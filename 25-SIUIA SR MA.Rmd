...
1. Meta-regression of proportion of patients included with exposure to prior aSAH as a continuous variable on rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up.

2. Meta-regression of proportion of patients included with ≤5mm aneurysms as a continuous variable on rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up.

3. Meta-regression of proportion of patients included with multiple aneurysms as a continuous variable on rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up.

4. Meta-regression of age as a continuous variable on rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up.

5. Sub-group analysis of pooled rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up with mean age categorised as ≤65-years or >65-years. 

6. Sub-group analysis of pooled rupture risk estimate per 100 SIUIAs ≤10mm managed with follow up with source population categorised as Japanese or non-Japanese. 

7. Sub-group analysis of study type per 100 SIUIAs ≤10mm managed with follow up categorised as prospective or retrospective.

8. Meta-regression of follow up time as a continuous variable on rupture risk estimate per 100 SIUIAs ≤10mm.
...
#### Sub-group analysis based on study age - Age < 60 vs Age 60 or greater 

```{r meta-analysis of proportions using glmm by age}
...
forest(pes.summary.glmm.age,
       ...
       text.addline2 = "Age >65 years mean follow up: 2.8 years",
       ) 
...
```{r metaregression japanese population}

pop.metareg = metaprop(rupt, total_size,
                      data=dat10,
                      studlab=paste(auth_year),
                      method="GLMM",
                      sm="PLOGIT",
...
pop.metareg.result <- metareg(pop.metareg, ~pop, hakn = TRUE)
...
forest(pes.summary.glmm.pop,
       ...
       text.addline2 = "Mean Non-Japanese study-level follow up: 3.8 years",
       ) 
...
dat.japan.age.metareg <- dat10 %>%
  mutate(age = coalesce(age_mean, age_med)) %>%
  select(auth_year, rupt, total_size, age, pop, fu, type)

```


```{r mv metaregression for pop as a continous variable, including all studies}
...
japan.age.metareg.result1 <- metareg(japan.age.metareg, pop, hakn = TRUE) 
...
japan.age.metareg.result2 <- metareg(japan.age.metareg, pop + age, hakn = TRUE) 

japan.age.metareg.result2

```

```{r mv metaregression for age + pop , including all studies }

japan.age.metareg.result5 <- metareg(japan.age.metareg, age + pop, hakn = TRUE) 

japan.age.metareg.result5

```


```{r mv metaregression for pop + fu as a continous variable, including all studies}

japan.age.metareg.result3 <- metareg(japan.age.metareg, pop + fu, hakn = TRUE) 

japan.age.metareg.result3
...
japan.age.metareg.result3 <- metareg(japan.age.metareg, pop + type, hakn = TRUE) 
...
japan.age.metareg.result2 <- metareg(japan.age.metareg, age * pop, hakn = TRUE)

japan.age.metareg.result2

```

```{r metaregression mv - option 2 - bubble plot, including all studies}

bubble(japan.age.metareg.result2, studlab=dat.japan.age.metareg$auth_year, xlim=c(0, 1), main="Meta-regression plot for mean age")
...
```{r forest for publication for sensitivity LOCF using GLMM, fig.height= 10, fig.width = 10}

forest(pes.summary.5locf,
       layout = "meta",
       comb.fixed = FALSE,
       comb.random = TRUE,
       print.I2.ci = TRUE,
...
```{r sensitivity analysis - consider leave-1-out on effect size - final studies}
...
forest(pes.sens.nos,
       ...
       xlab = "Rupture per 100 aneurysms",
       pooled.events = TRUE,
       text.addline1 = "Mean study-level follow up: 5.7-years",
       JAMA.pval = TRUE,
       ) 
