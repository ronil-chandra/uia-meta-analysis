...
```{r calculate total number of aneurysms for specific size criteria 10 mm and less}
...
dat <- sizedata %>%
  ...
  mutate(pop = fct_collapse(dat$country, 
             "Japanese" = "Japan",
...
2. Aneurysm mutiplicity is not taken into account, to avoid multiple assumptions regarding proportion with prior SAH and proportion with multiple aneurysms and to avoid greater inaccuracy in this analysis. 
3. Thus it is assumed for this analysis that each patient has 1 aneurysm. 
4. Data analysis is limited to studies in which there are known or calculated values of ai, bi, ci, and di using the assumptions. If there are NAs in the 2 x 2 table, then that study is excluded. 
