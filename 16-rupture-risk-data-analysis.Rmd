...
1. If the number of aneurysms for a particular size criteria in the study has been reported, this has been extracted and utilised for analysis. 
1. If the number of number of aneuryms is not known for the specific size criteria, the assumption is that a patient with multiple aneurysms has 2 aneurysms. 
2. This can be caculated using the proportion of patients with multiple aneuryms *variable multi_tot* in the total observation cohort *variable num_tot*, and multiplying it by the number of patients in that specific size criteria. This is the number of patients that we can assume has 2 aneurysms. 
3. The total number of aneurysms is then calculated as *variable num_multi*
